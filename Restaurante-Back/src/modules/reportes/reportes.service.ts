...
export class ReportesService {
    ...
    async generarTicketZPdf(cierreCajaId: string): Promise<Readable> {
       const cierre = await this.cierreCajaRepository.findOne({
        ...
        const resumen = [
            ...
            { label: 'Total Recaudado (Pagos):', value: Number(cierre.total_recaudado) },
            { label: 'Total Propina:', value: Number(cierre.total_propina) }, 
        ];
    ...
    const cuadre = [
        ...
        { label: 'Recaudo en Efectivo (Venta + Propina):', value: Number(cierre.total_pagos_efectivo)}, 
        { label: 'Ingresos Extra:', value: totalIngresosExtra },
        { label: 'Gastos Operacionales:', value: Number(parseFloat(cierre.gastos_operacionales.toString())* -1)  },
    ];
    cuadre.forEach((item) => {
        y = this.writeRow(doc, item.label, item.value, y);
    });
    ...
    const saldoEsperadoCorregido =
        parseFloat(cierre.saldo_inicial_caja.toString()) + parseFloat(cierre.total_pagos_efectivo.toString()) + parseFloat(totalIngresosExtra.toString()) - parseFloat(cierre.gastos_operacionales.toString());
    ...
    async generarTicketXPdf(establecimientoId: string, usuarioCajeroId: string): Promise<Readable> {
       ...
       const pagosDelTurno = await this.pagoRepository
            ...
            .where('factura.establecimiento_id = :establecimientoId', { establecimientoId })
            .andWhere('factura.usuario_cajero_id = :usuarioCajeroId', { usuarioCajeroId })
            .andWhere('pago.cierre_caja_id = :caja_id', { caja_id: cierreActivo.id })
            .getMany();
        ...
        const saldoEsperadoActual = parseFloat(cierreActivo.saldo_inicial_caja.toString()) + parseFloat(cierreActivo.total_pagos_efectivo.toString()) + parseFloat(totalIngresosExtra.toString()) - parseFloat(cierreActivo.gastos_operacionales.toString())
        ...
        doc.text('---', this.MARGIN, y, { align: 'center' });
        y += 10;
        y = this.writeRow(doc, 'TOTAL RECAUDADO (VENTAS + PROPINA):', Number(cierreActivo.total_neto_ventas), y, true);
        y = this.writeRow(doc, 'Ventas sin Propina:', Number(cierreActivo.total_recaudado), y);
        y = this.writeRow(doc, 'Propina Recaudada:', Number(cierreActivo.total_propina), y);
        doc.text('---', this.MARGIN, y, { align: 'center' });
        y += 10;
        doc.font('Helvetica-Bold').text('DETALLE DE RECAUDO POR PAGO (INCL. PROPINA):', this.MARGIN, y);
        y += 30;
        ...
        y = this.writeRow(doc, 'Pagos en Efectivo:',  Number(cierreActivo.total_pagos_efectivo), y);
        y = this.writeRow(doc, 'Ingresos Extra:', totalIngresosExtra, y);
        y = this.writeRow(doc, 'Gastos Operacionales:', cierreActivo.gastos_operacionales * -1, y);
        doc.text('---', this.MARGIN, y, { align: 'center' });
        y += 10;

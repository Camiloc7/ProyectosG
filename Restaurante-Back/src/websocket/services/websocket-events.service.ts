...
public emitPrintJobToDevice(
  ...
  if (this.server) {
    this.server.to(`establecimiento-${establecimientoId}`).emit("printJob", payload); 
    this.logger.log(`[WS] Tarea 'printJob' enviada a establecimiento-${establecimientoId} para tipo: ${payload.tipoImpresion}`);
  } else {
    this.logger.error('No se pudo emitir la tarea de impresión: el servidor Socket.IO no está disponible.');
  }
}

public emitCajaStatusUpdate(
  establecimientoId: string,
  usuarioCajeroId: string,
  cajaAbierta: boolean
): void {
  if (this.server) {
    const eventName = 'cajaStatusUpdated'; 
    const payload = { usuarioCajeroId, cajaAbierta };
    this.server
      .to(`establecimiento-${establecimientoId}`)
      .emit(eventName, payload);

    this.logger.log(
      `[WS] Estado de Caja actualizado: Cajero ${usuarioCajeroId} en ${establecimientoId}. Abierta: ${cajaAbierta}`
    );
  }
}

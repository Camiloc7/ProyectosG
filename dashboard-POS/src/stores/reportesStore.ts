...
export interface VentasPorDiaHora {
    diaSemana: number; 
    horaDia: number; 
    totalVentas: number;
}
...
export const useReportesDashboardStore = create<ReportesDashboardState>(
  (set) => ({
    reporteVentas: null,
...
      isComparison = false
    ) => {
      console.warn("fecha fin", fechaFin);
      console.warn("fechaInicio", fechaInicio);
      console.warn("usuarioCajeroId", usuarioCajeroId);
      console.warn("limit", limit);
      console.warn("offset", offset);
      console.warn("isComparison", isComparison);

      set((state) => ({
        loading: { ...state.loading, ventas: true },
        error: { ...state.error, ventas: null },
      }));
...
          error: {
            ...state.error,
            ventas: "No se encontró el token de autenticación.",
          },
        }));
        toast.error(
...
      }
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            responseData.message || "Error al generar el reporte de ventas."
          );
        } 
        ...
        set((state) => ({
          error: {
            ...state.error,
            ventas: error.message || "Error al generar el reporte.",
          },
          reporteVentas: null,
          reporteVentasComparacion: null,
        }));
        toast.error(
          error.message || "No se pudo generar el reporte de ventas."
        );
      } finally {
...
      set((state) => ({
        loading: { ...state.loading, inventario: true },
        error: { ...state.error, inventario: null },
      }));
      const token = useAuthStore.getState().token;

      if (!token) {
        set((state) => ({
          loading: { ...state.loading, inventario: false },
          error: {
            ...state.error,
            inventario: "No se encontró el token de autenticación.",
          },
        }));
        toast.error(
          "Tu sesión ha expirado. Por favor, inicia sesión de nuevo."
...
        if (!res.ok) {
          throw new Error(
            responseData.message || "Error al generar el reporte de inventario."
          );
        }
        set({
...
            ...state.error,
            inventario:
              error.message || "Error al generar el reporte de inventario.",
          },
          reporteInventario: null,
        }));
        toast.error(
          error.message || "No se pudo generar el reporte de inventario."
        );
      } finally {
...
      set((state) => ({
        loading: { ...state.loading, pedidos: true },
        error: { ...state.error, pedidos: null },
      }));
      const token = useAuthStore.getState().token;

      if (!token) {
        set((state) => ({
          loading: { ...state.loading, pedidos: false },
          error: {
            ...state.error,
            pedidos: "No se encontró el token de autenticación.",
          },
        }));
        toast.error(
          "Tu sesión ha expirado. Por favor, inicia sesión de nuevo."
...
        if (!res.ok) {
          throw new Error(
            responseData.message || "Error al generar el reporte de pedidos."
          );
        }
        set({
...
          error: {
            ...state.error,
            pedidos: error.message || "Error al generar el reporte de pedidos.",
          },
          reportePedidos: null,
        }));
        toast.error(
          error.message || "No se pudo generar el reporte de pedidos."
        );
      } finally {

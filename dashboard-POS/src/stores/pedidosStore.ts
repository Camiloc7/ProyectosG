...
type PedidosState = {
  ...
  traerPedidosPaginados: (
    ...
    fechaFin?: string,
    estado?: string
  ) => Promise<IPedidos[] | null>;
  actualizarEstadoPedido: (id: string, estado: string) => Promise<void>;
  agregarItemsAPedidoEntregado: (idPedido: string, data: any) => Promise<any>;
};
...
export const formatearPedidos = (dataOriginal: any[]): IPedidos[] => {
  return dataOriginal.map((pedido) => ({
    id: pedido.id,
    created_at: pedido.created_at, // <-- added
...
export const usePedidosStore = create<PedidosState>((set, get) => ({
  ...
  agregarItemsAPedidoEntregado: async (idPedido, data: any) => {
    set({ loading: true });
    const token = useAuthStore.getState().token;
    console.warn(data);
    try {
      const res = await fetch(`${RUTA}/pedidos/${idPedido}/items/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,

...
export const generarPayloadPago = async ({
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
        try {
            const respaldoUnicoStr = window.localStorage.getItem(`respaldo_unico_${pedidoId}`);
            if (respaldoUnicoStr) {
                respaldoUnico = JSON.parse(respaldoUnicoStr);
            }

            const respaldoDivididoStr = window.localStorage.getItem(`respaldo_dividido_${pedidoId}`);
    ...
    return {
        payload,
        respaldoKey: respaldoUnico ? `respaldo_unico_${pedidoId}` : `respaldo_dividido_${pedidoId}`,
...
// // type MetodoPago = 'efectivo' | 'electronico'
...
//   denominacionesEfectivo
// }: Params): Promise<any | null> => {
//   //Sacamos la data que se haya guardado en el disco

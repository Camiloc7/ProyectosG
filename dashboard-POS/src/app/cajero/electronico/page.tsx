...
export default function PagoElectronico() {
  ...
  const pintarDatosDelPedido = async () => {
    if (!idPedido) {
      router.push("/cajero/pagar?pedidoId=" + pedidoPorPagar?.id);
  ...
  const totalIngresado = cuentasSeleccionadas.reduce(
    (sum, c) => sum + (c.monto || 0),
    0
  );
  ...
  const handleClosePdfModal = async () => {
    ...
    // router.push(`pagar/${pedidoPorPagar?.id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (dividirPago) {
        ...
        const totalCuentas = cuentasConMonto.reduce(
          (sum, c) => sum + c.monto,
          0
        );
        ...
        if (totalIngresado < totalAPagar || totalIngresado > totalAPagar + 1) {
          toast.error(
            "El total ingresado no coincide con el total a pagar (tolerancia de 1 peso)."
          );
          return;
        }
        ...
        for (const c of cuentasConMonto) {
          ...
          const facturaTraida: FacturaEntity | null = await pagarPedido(
            resultCuenta
          );
  ...
  const inValido =
    totalIngresado < totalAPagar || totalIngresado > totalAPagar + 1;
  return (
    <div
      style={{
        minHeight: "100vh",
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            {!dividirPago ? (
                ...
                {cuentasConEfectivo.map((c) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border:
                        cuenta === c.id ? "2px solid orange" : "1px solid #ccc",
                      boxShadow:
                        cuenta === c.id
              ...
              >
                {cuentasConEfectivo.map((c) => {
                  const seleccionada = cuentasSeleccionadas.find(
                    (cs) => cs.id === c.id
                  );
                  return (
                    <div
                    ...
                    >
                      ...
                      {seleccionada && (
                        <input
                          ...
                          value={
                            seleccionada.monto === 0 ? "" : seleccionada.monto
                          }
                          min={0}
                          max={totalAPagar}
                          onClick={(e) => e.stopPropagation()}
                          onChange={(e) =>
                            handleMontoChange(c.id, Number(e.target.value))
                          }
                ...
                <div style={{ marginTop: 16, fontWeight: 400, color: "#333" }}>
                  Total ingresado:{" "}
                  <span
                    style={{
                      fontWeight: 600,

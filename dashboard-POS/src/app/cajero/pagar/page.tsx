...
export default function DivisionCuentas() {
    ...
    const pedidoPorPagar: IPedidos | undefined = pedidos.find(p => p.id === pedidoId);
    const [pedido, setPedido] = useState<IPedidos>();
    const [active, setActive] = useState<number>(0);
    const [moneyCount, setMoneyCount] = useState<number>(2);
    const [descuento, _setDescuento] = useState<number>(0);
    ...
    useEffect(() => {
        ...
        // Si necesitas transformar los items, hazlo aquí
        setPedido(pedidoPorPagar);
    }, [pedidoPorPagar, router]);
            ...
            prev.map((e, i) =>
                i === idx
                    ? {
                        ...
                        correo: !correctedDivision.correo?.trim() || !isEmailValid(correctedDivision.correo || '')
                    }
                    : e
            )
        );
    ...
    const validateItemDistribution = (): boolean => {
        ...
        for (const item of pedidoPorPagar.pedidoItems) {
            if ((totalAssigned[item.id] || 0) > item.cantidad) {
                toast.error(
                    `Error: El producto "${item.nombre}" tiene asignadas más unidades (${totalAssigned[item.id]}) que las disponibles (${item.cantidad}).`
                );
                return false;
            }
        }
    ...
    const handleProcess = async () => {
        ...
        if (active === 0) {
            ...
            localStorage.setItem(`respaldo_unico_${pedidoPorPagar.id}`, JSON.stringify(data));
        } else {
            if (!validateAll() || !validateItemDistribution()) return;
            ...
            const pagos = divisiones.map((d) => {
                ...
                const baseConDescuento = base * (1 - descuento / 100);
                ...
                if (totalWithTip <= 0) {
                    toast.error('No puedes continuar, hay tarjetas sin monto a pagar asignado.');
                    return null;
                }
            ...
            localStorage.setItem(`respaldo_dividido_${pedidoPorPagar.id}`, JSON.stringify(data));
        }
        ...
        >
            ...
            <div style={{ maxWidth: 1124, margin: '0 auto' }}>
                ...
                <div className="flex items-center gap-4 mb-8">
                    <div
                        className="relative flex flex-1 p-1 rounded-xl bg-black"
                    >
                        <div
                            className="absolute inset-0 z-0 transition-transform duration-300 ease-out rounded-lg"
                            style={{
                                width: `${100 / ['Una Sola Cuenta', 'Dividir por Productos', 'Dividir por Dinero'].length}%`,
                                transform: `translateX(${active * 100}%)`,
                                backgroundColor: ORANGE,
                                boxShadow: '0 2px 5px rgba(0, 0, 0, 0.1)',
                            }}
                        />
                        {['Una Sola Cuenta', 'Dividir por Productos', 'Dividir por Dinero'].map((opt, i) => (
                            <button
                                key={opt}
                                onClick={() => {
                                    if (!locked) {
                ...
                `}
                    ...
                    >
                        {locked ? <Lock size={24} color={ORANGE} strokeWidth={2.5} /> : <Unlock size={24} color={ORANGE} />}
                    </button>
                </div>
                {mode === 'MONEY' && (
                    <div style={{ marginBottom: 30 }}>
                        ...
                        <input
                            ...
                            value={moneyCount}
                            onChange={(e) => setMoneyCount(Math.max(0, Math.min(400, Number(e.target.value))))}
                        />
                    </div>
                )}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                <span style={{ fontSize: 18, fontWeight: 600, color: ok ? '#38A169' : '#E53E3E' }}>
                                    {active === 0
                                        ...
                                        : `${formatearNumero(totalConPropina)} / ${formatearNumero(subtotalConDescuento)}`}
                                </span>

...
import { Eye, FileText, ChevronLeft, ChevronRight, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
import * as Tooltip from "@radix-ui/react-tooltip";

export default function Pedidos() {
  const { loading, pedidos, pedidosTotal, traerPedidosPaginados } = usePedidosStore();
  const { getFacturaPdf, getFacturaByPedidoId } = useFacturasStore();
  ...
  useEffect(() => {
    traerPedidosPaginados(page, itemsPerPage, search, fechaInicio, fechaFin, estadoFiltro);
  }, [page, itemsPerPage, search, fechaInicio, fechaFin, estadoFiltro, traerPedidosPaginados]);
  ...
  const handleFacturaClick = async (pedido: IPedidos) => {
    if (pedido.estado.toUpperCase() !== "PAGADO") {
      alert("Este pedido no tiene una factura asociada.");
      return;
    }
    const factura = await getFacturaByPedidoId(pedido.id);
    if (factura && factura.id) {
      await getFacturaPdf(factura.id);
    } else {
  ...
  const getSortIcon = (column: string) => {
    if (sortColumn !== column) return <ArrowUpDown className="ml-2 w-4 h-4 text-gray-400" />;
    return sortDirection === 'asc' ? <ArrowUp className="ml-2 w-4 h-4" /> : <ArrowDown className="ml-2 w-4 h-4" />;
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-[calc(100vh-4rem)] text-gray-800">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        {selectedPedido && (
          <BotonRestaurante label="Volver a la lista" onClick={handleBackToList} />
        )}
        <div className="flex flex-wrap gap-3">
          <div className="flex flex-col">
            <label htmlFor="search" className="text-sm font-medium text-gray-700">Buscar por ID</label>
            <input
              id="search"
              type="text"
          ...
          {selectedPedido ? (
            ...
            <>
              <div className="mb-4 flex items-center gap-2">
                <label htmlFor="estadoFiltro" className="text-sm font-medium text-gray-700">Filtrar por Estado:</label>
                <select
                  id="estadoFiltro"
                  value={estadoFiltro}
                  onChange={(e) => {
              ...
              <div className="overflow-x-auto bg-white rounded-lg shadow-md">
                <table className="min-w-full table-auto text-sm text-gray-700">
                  <thead className="bg-gray-100 border-b border-gray-200">
                    <tr>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider cursor-pointer" onClick={() => handleSort('codigo_pedido')}>
                        <div className="flex items-center">
                          ID
                          {getSortIcon('codigo_pedido')}
                        </div>
                      </th>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider">Mesa</th>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider">Tipo</th>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider">Estado</th>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider cursor-pointer" onClick={() => handleSort('total_estimado')}>
                        <div className="flex items-center">
                          Total
                          {getSortIcon('total_estimado')}
                        </div>
                      </th>
                      <th className="px-6 py-3 text-left font-bold uppercase tracking-wider cursor-pointer" onClick={() => handleSort('created_at')}>
                        <div className="flex items-center">
                          Fecha
                          {getSortIcon('created_at')}
                        </div>
                      </th>
                      <th className="px-6 py-3 text-center font-bold uppercase tracking-wider">Acciones</th>
                    </tr>
                  </thead>
                  <tbody>
                    {/* Se usa 'sortedPedidos' que es el resultado del sort local */}
                    {sortedPedidos.map((pedido) => (
                      <tr key={pedido.id} className="border-b border-gray-200 last:border-0 hover:bg-amber-50 transition-colors">
                        <td className="px-6 py-4">{pedido.codigo_pedido?.slice(-3) ?? 'N/A'}</td>
                        <td className="px-6 py-4">{pedido.mesa_numero || 'N/A'}</td>
                        <td className="px-6 py-4">{pedido.tipo_pedido}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            pedido.estado === "PAGADO" ? "bg-green-100 text-green-800" :
                            pedido.estado === "ABIERTO" ? "bg-blue-100 text-blue-800" :
                            "bg-red-100 text-red-800"
                          }`}>
                            {pedido.estado}
                          </span>
                        </td>
                        <td className="px-6 py-4 font-semibold">${pedido.total_estimado.toFixed(2)}</td>
...
// import * as Tooltip from "@radix-ui/react-tooltip";
// export default function Pedidos() {
//   const { loading, pedidos, traerTodosLosPedidos } = usePedidosStore();
//   const { getFacturaPdf, getFacturaByPedidoId } = useFacturasStore();
//   const [search, setSearch] = useState("");
//   const [page, setPage] = useState(1);
//   const [itemsPerPage, setItemsPerPage] = useState(25);
...
//       .filter((p) => {
//         if (estadoFiltro !== "TODOS" && p.estado.toUpperCase() !== estadoFiltro) {
//           return false;
//         }
//         return true;
//       })
...
//         );
//       });
//   }, [sortedPedidos, search, fechaInicio, fechaFin, estadoFiltro]);
//   const totalPages = Math.ceil(pedidosFiltrados.length / itemsPerPage);
//   const paginados = useMemo(() => {
//     const start = (page - 1) * itemsPerPage;
//     const end = start + itemsPerPage;
//     return pedidosFiltrados.slice(start, end);
//   }, [pedidosFiltrados, page, itemsPerPage]);
//   const getSortIcon = (column: string) => {
//     if (sortColumn !== column) return <ArrowUpDown className="ml-2 w-4 h-4 text-gray-400" />;
//     return sortDirection === 'asc' ? <ArrowUp className="ml-2 w-4 h-4" /> : <ArrowDown className="ml-2 w-4 h-4" />;
//   };
...
//           <div className="flex flex-col">
//             <label htmlFor="fechaInicio" className="text-sm font-medium text-gray-700">Fecha de Inicio</label>
//             <DatePickerInput value={fechaInicio} onChange={setFechaInicio} />
//           </div>
//           <div className="flex flex-col">
//             <label htmlFor="fechaFin" className="text-sm font-medium text-gray-700">Fecha de Fin</label>

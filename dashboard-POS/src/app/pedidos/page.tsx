...
export default function Pedidos() {
  ...
  const exportExcel = async () => {
    try {
      ...
      // 👇 Esperamos a que se traigan todos los pedidos
      const pedidosData = await traerPedidosPaginados(
        1,
        5000, // o el número que quieras traer
        search,
        fechaInicio,
        fechaFin,
        estadoFiltro
      ...
      if (!pedidosData?.length) {
        toast.error("No se encontraron pedidos para exportar.");
        return;
      }

      // 🔥 Ya podés generar tu Excel con pedidosData
      const data = pedidosData.map((pedid) => ({
        Mesa: pedid?.mesa_numero || "N/A",
        Tipo: pedid?.tipo_pedido || "N/A",
        Estado: pedid.estado || "N/A",
        Total: pedid.total_estimado || "N/A",
        Fecha: formatDate(pedid.created_at),
        Hora: formatTime(pedid.created_at),
        "Nombre del Cliente": pedid.cliente_nombre || "N/A",
        "Código del Pedido": pedid.codigo_pedido || "N/A",
        ID: pedid.id || "N/A",
      }));

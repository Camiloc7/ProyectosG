...
export default function Configuracion() {
  ...
  const validateImpresoraForm = () => {
    ...
    if (!impresoraForm.ip_address.trim()) newErrors.ip_address = true;
    if (!impresoraForm.puerto || Number(impresoraForm.puerto) <= 0)
      newErrors.puerto = true;
    if (!impresoraForm.tipo_impresion) newErrors.tipo_impresion = true;

    // Validar formato IP
    const ipRegex =
      /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
    if (impresoraForm.ip_address && !ipRegex.test(impresoraForm.ip_address)) {
      newErrors.ip_address = true;
    }
  ...
  const handleEliminarImpresora = async (id: string) => {
    const confirmado = await confirm({
      title: "¿Deseas eliminar esta impresora?",
      description: "La accion es permanente",
  ...
  const editarImpresora = (impresora: IImpresora) => {
    setImpresoraForm({
      nombre: impresora.nombre,
      ip_address: impresora.ip_address,
      puerto: String(impresora.puerto),
      tipo_impresion: impresora.tipo_impresion,
      activa: impresora.activa,
    });
    setEditandoImpresora(impresora.id); // ← Aquí está la clave
  };
  ...
  const agregarOEditarImpresora = () => {
    ...
    const nuevaImpresora: IImpresora = {
      ...
      nombre: impresoraForm.nombre.trim(),
      ip_address: impresoraForm.ip_address.trim(),
      puerto: Number(impresoraForm.puerto),
    ...
    if (editandoImpresora) {
      actualizarImpresora(ImpresoraSinId, nuevaImpresora.id);
    } else {
      crearImpresora(ImpresoraSinId);
    }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="w-full space-y-6">
        ...
        {/* Sección de configuración de impresoras */}
        <div className="bg-white p-6 rounded-xl shadow-sm">
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputFieldDos
                  ...
                  onChange={(e) =>
                    onImpresoraFormUpdate("nombre", e.target.value)
                  }
                  error={impresoraErrors.nombre}
                  placeholder="Ej: Impresora Cocina 1"
                />

                <InputFieldDos
                  label="Dirección IP"
                  value={impresoraForm.ip_address}
                  onChange={(e) =>
                    onImpresoraFormUpdate("ip_address", e.target.value)
                  }
                  error={impresoraErrors.ip_address}
                ...
                <InputFieldDos
                  label="Puerto"
                  type="number"
                  value={impresoraForm.puerto}
                  onChange={(e) =>
                    onImpresoraFormUpdate("puerto", e.target.value)
                  }
                  error={impresoraErrors.puerto}
                  placeholder="9100"
                />
                ...
                <InputFieldDos
                  ...
                  onChange={(e) =>
                    onImpresoraFormUpdate("tipo_impresion", e.target.value)
                  }
                  error={impresoraErrors.tipo_impresion}
                />
              </div>
              ...
              <div className="flex items-center gap-2 mt-4 mb-4">
                <Checkbox
                  ...
                  onChange={(e) => onImpresoraFormUpdate("activa", e)}
                  checked={impresoraForm.activa}
                />
              </div>
              ...
              <div className="flex gap-2">
                <BotonRestaurante
                  label={
                    editandoImpresora ? "Guardar Cambios" : "Agregar Impresora"
                  }
                  ...
                  impresoras.map((impresora) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex-1">
                            <h4
                              className={`font-semibold flex items-center gap-2 ${
                                isEditing ? "text-blue-800" : "text-gray-800"
                              }`}
                            >
                              {impresora.nombre}
                              <span
                            ...
                            >
                              <p>
                                <strong>IP:</strong> {impresora.ip_address}
                              </p>
                              <p>
                                <strong>Puerto:</strong> {impresora.puerto}
                              </p>
                              <p>
                                <strong>Tipo:</strong>
                                <span
                        ...
                        {/* Barra de progreso sutil cuando está editando */}
                        {isEditing && (

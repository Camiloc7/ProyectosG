...
const SimpleSelect: React.FC<SimpleSelectProps> = ({
  ...
  const handleSelectOption = (option: SimpleSelectOption) => {
    if (disabled || readOnly) return; // 👈 evita selección
    const selectedValue =
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (disabled || readOnly || !searchBarIsOpen) return; // 👈 bloquea teclas
  ...
  return (
    <div className={`w-[${width}] ${label ? "mb-4 mt-4" : ""}`}>
      {label && (
        <label
          style={{
            display: "block",
      ...
      <div ref={ref} style={{ position: "relative" }}>
        <input
          ...
          readOnly // 👈 siempre readOnly para que no se pueda tipear
          disabled={disabled}
          style={{
            height,
            padding: "0 16px",
            border: `1px solid ${
              disabled ? "#A0AEC0" : error ? COLOR_ERROR : ORANGE
            }`,
            borderRadius: 25,
            fontSize: 14,
            color: disabled || readOnly ? "#A0AEC0" : COLOR_TEXTO, // 👈 se ve distinto si es readOnly
            width: "100%",
        ...
        <span
          ...
          style={{
            position: "absolute",
            color: disabled || readOnly ? "#A0AEC0" : ORANGE,
            right: 16,
            top: "50%",

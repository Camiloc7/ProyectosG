...
export const LineChartCard = ({ title, data, dataKey, xLabel, yLabel, series }: ChartCardProps) => (
    <div className="bg-gray-50 p-6 rounded-xl shadow border border-gray-100">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">{title}</h3>
        <ResponsiveContainer width="100%" height={300}>
            <LineChart
            ...
            >
                ...
                {series && series.length > 0 ? (
                    series.map((s, idx) => (
                        <Line key={s.dataKey} type="monotone" dataKey={s.dataKey} stroke={s.stroke || COLORS[idx % COLORS.length]} activeDot={{ r: 6 }} name={s.name || s.dataKey} />
                    ))
                ) : (
                    dataKey && <Line type="monotone" dataKey={dataKey} stroke="#8884d8" activeDot={{ r: 8 }} />
                )}
            </LineChart>
        </ResponsiveContainer>
    </div>

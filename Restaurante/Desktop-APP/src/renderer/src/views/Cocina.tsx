...
export default function Cocina() {
  ...
  const { pedidosCocina, loading, traerListaPedidos, actualizarEstadoPedido } = usePedidosStore()
  ...
  useEffect(() => {
    async function init() {
      ...
      socket.on('pedidoCreated', ({ pedidoId }) => {
        ...
        usePedidosStore.getState().traerListaPedidos()
      })
      ...
      socket.on('pedidos_actualizados', (nuevosPedidos: any[]) => {
        ...
        usePedidosStore.setState({ pedidosCocina: pedidosFiltrados })
      })
    }
  ...
  const getButtonVariacion = (estado: string): 'default' | 'claro' | 'peligro' | 'verde' => {
    switch (estado) {
      case 'EN_PREPARACION':
        return 'verde' // ✅ Botón verde
      default:
        return 'default' // Botón naranja normal
    }
  }
          ...
          <div style={{ marginBottom: 48 }}>
            ...
            >
              ...
              >
                ...
                <span style={{ fontWeight: 600 }}>No hay pedidos por preparar</span>
              </div>
            </div>
          </div>
        )}
            ...
            {pedidosCocina.map((pedido) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ margin: 0, fontSize: 14, color: '#374151', marginBottom: 12 }}>
                    {getTitulo()}
                  </p>

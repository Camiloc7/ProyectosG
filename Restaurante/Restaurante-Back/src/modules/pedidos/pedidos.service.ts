...
export class PedidosService {
  private readonly ESTABLECIMIENTO_TIMEZONE = 'America/Bogota';
  constructor(
...
private convertPedidoDatesToEstablishmentTimezone(
    pedido: PedidoEntity,
  ): PedidoEntity {
    if (pedido.created_at) {
      pedido.created_at = toZonedTime(
        pedido.created_at,
        this.ESTABLECIMIENTO_TIMEZONE,
      );
    }
    if (pedido.updated_at) {
      pedido.updated_at = toZonedTime(
        pedido.updated_at,
        this.ESTABLECIMIENTO_TIMEZONE,
      );
    }
    if (pedido.fecha_ultima_actualizacion_relevante_cocina) {
      pedido.fecha_ultima_actualizacion_relevante_cocina = toZonedTime(
        pedido.fecha_ultima_actualizacion_relevante_cocina,
        this.ESTABLECIMIENTO_TIMEZONE,
      );
    }
    if (pedido.fecha_cancelacion) {
      pedido.fecha_cancelacion = toZonedTime(
    ...
    if (pedido.pedidoItems) {
      pedido.pedidoItems = pedido.pedidoItems.map((item) => {
        if (item.fecha_hora_estado_cocina_cambio) {
          item.fecha_hora_estado_cocina_cambio = toZonedTime(
            item.fecha_hora_estado_cocina_cambio,
            this.ESTABLECIMIENTO_TIMEZONE,
          );
        }
        return item;
...
const totalEstimado = await this.calcularTotalEstimado(preprocessedItems, establecimientoId);
    const MAX_RETRIES = 3;
    let retries = 0;
    const ESTABLECIMIENTO_TIMEZONE = 'America/Bogota';     
    while (retries < MAX_RETRIES) {
      ...
      try {
        ...
        const nowInEstablishmentTimezone = toZonedTime(nowUtc, ESTABLECIMIENTO_TIMEZONE);
        const startOfTodayForQuery = startOfDay(nowInEstablishmentTimezone);
        const ultimoPedidoHoy = await queryRunner.manager
          .createQueryBuilder(PedidoEntity, 'pedido')
          .where('pedido.created_at >= :startOfToday', { startOfToday: startOfTodayForQuery }) 
        ...
        const codigoPedido = `PED-${format(nowInEstablishmentTimezone, 'yyyyMMdd')}-${String(nextNumero).padStart(3, '0')}`;
        const pedido: PedidoEntity = this.pedidoRepository.create({
          ...
          notas: notas || null,
          fecha_ultima_actualizacion_relevante_cocina: nowUtc,
        ...
        const pedidoItemsEntities = preprocessedItems.map((item) =>
          this.pedidoItemRepository.create({
            ...
            fecha_hora_estado_cocina_cambio: nowUtc,
          }),
        );
        ...
        if (pedidoCompleto) {
          try {
            await this.impresionService.enviarComandaAImpresora(pedidoCompleto, establecimientoId);
          } catch (error) {
            console.error(
...
public async findAll(
    ...
    return pedidos.map(pedido => this.convertPedidoDatesToEstablishmentTimezone(pedido));
  }
...
public async findOne(
    ...
    return this.convertPedidoDatesToEstablishmentTimezone(pedido);
  }
  ...
  public async update(
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(`No se puede actualizar un pedido en estado "${pedido.estado}". Solo se pueden editar pedidos 'ABIERTO', 'EN_PREPARACION', 'LISTO_PARA_ENTREGAR', 'ENTREGADO' o 'ENVIADO_A_COCINA'.`);
      }

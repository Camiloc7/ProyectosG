...
import { ConciliateStockDto } from './dto/conciliate-stock.dto';
...
async uploadExcel(
  ...
  @Roles(RoleName.ADMIN, RoleName.SUPERVISOR, RoleName.COCINERO, RoleName.MESERO, RoleName.CAJERO)
  ...
  @Patch(':id/conciliar')
async conciliateStock(
    @Param('id') id: string,
    @Body() conciliateDto: ConciliateStockDto,
    @Req() req: AuthenticatedRequest,
) {
    const establecimientoId = req.user.establecimiento_id;
    const ingrediente = await this.ingredientesService.conciliateStock(
        id,
        conciliateDto.newStock,
        establecimientoId,
    );
    return { 
        message: 'Stock conciliado exitosamente', 
        data: ingrediente 
    };
}

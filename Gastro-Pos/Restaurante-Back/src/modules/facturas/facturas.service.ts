...
//             const pdfBuffer = await this.enviarFacturaAPIExterna(
//                 savedFactura,
...
// this.impresionService.enviarFacturaTiquetaImpresora(savedFactura, pedidoCompletoParaApi, establecimientoId).catch(console.error); 
...
// Asegúrate de tener inyectado el servicio de WebSockets, por ejemplo:
// constructor(
//   // ... otros servicios
//   private readonly websocketEventsService: WebsocketEventsService,
// ) {}
...
        }
        
        // --- Lógica de Envío a API Externa y Actualización de PDF ---
        
        try {
            const establecimientoParaApi = await this.establecimientosService.findOne(establecimientoId);
            const usuarioCajeroParaApi = await this.usuariosService.findOne(usuarioCajeroId, establecimientoId);
...
                savedFactura.pdf_factura_data = pdfBuffer;
                savedFactura.estado_envio_api = EstadoEnvioFactura.ENVIADO;
// LÍNEA ELIMINADA: this.impresionService.enviarFacturaTiquetaImpresora(savedFactura, pedidoCompletoParaApi, establecimientoId).catch(console.error); 
...
        }
        
        // [NUEVO FLUJO] Notificamos al sistema local que una factura fue creada y está lista para imprimirse
        // Utilizamos la ID de la factura para que el frontend pueda obtener la data necesaria.
...
    }
    ...
    private async enviarFacturaAPIExterna(
        ...
        try {
            ...
            console.error('[enviarFacturaAPIExterna] Error al enviar la factura a la API externa:', error.message);
            if (axios.isAxiosError(error) && error.response) {
                console.error('Respuesta de error de la API externa:', JSON.stringify(error.response.data, null, 2));
            }
            throw error;
        }
    }

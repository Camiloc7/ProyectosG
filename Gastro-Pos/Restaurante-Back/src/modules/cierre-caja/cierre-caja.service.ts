...
export class CierreCajaService {
  ...
  async abrirCaja(createCierreCajaDto: CreateCierreCajaDto): Promise<CierreCajaEntity> {
    ...
    return this.cierreCajaRepository.save(nuevoCierre);
  }
...
      cierreCaja.total_pagos_efectivo = pagosDelTurno
...
      cierreCaja.total_pagos_tarjeta = pagosDelTurno
        .filter(pago => pago.cuentaBancaria?.medio_pago_asociado?.nombre.toUpperCase().includes('TARJETA'))
        .reduce((sum, pago) => {
          const montoAPlicar = pago.facturaPagosCliente ? Number(pago.facturaPagosCliente.monto_pagado) : 0;
          return sum + montoAPlicar;
        }, 0);

      cierreCaja.total_pagos_otros = pagosDelTurno
        .filter(pago => pago.cuentaBancaria?.medio_pago_asociado &&
          !pago.cuentaBancaria.medio_pago_asociado.es_efectivo &&
          !pago.cuentaBancaria.medio_pago_asociado.nombre.toUpperCase().includes('TARJETA'))
        .reduce((sum, pago) => {
          const montoAPlicar = pago.facturaPagosCliente ? Number(pago.facturaPagosCliente.monto_pagado) : 0;
          return sum + montoAPlicar;
        }, 0);

      cierreCaja.total_recaudado = pagosDelTurno.reduce((sum, pago) => {
        const montoAPlicar = pago.facturaPagosCliente ? Number(pago.facturaPagosCliente.monto_pagado) : 0;
        return sum + montoAPlicar;
      }, 0);
...
async cerrarCajasPendientes(): Promise<void> {
    try {
        const turnosAbiertos = await this.findAllOpen(); 
...
      const fechaCierre = new Date();
      ...
      for (const pedido of pedidosAActualizar) {
        // pedido.notas = 'Movido a pendiente de pago por cierre de caja automático.';
...
    }
  }
 async findAllOpen(): Promise<CierreCajaEntity[]> {

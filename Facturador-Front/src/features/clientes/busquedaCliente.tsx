...
import { validateTextos } from '../../helpers/validacionesDeBusqueda'; // Importamos la validación
...
const tiposDeBusqueda: string[] = ['Nombre', 'Numero de identificacion'];

const BusquedaClienteModal: React.FC<ModalFormProps> = ({
  isOpen,
  onClose,
  onSubmit,
}) => {
  const [tipoDeBusqueda, setTipoDeBusqueda] = useState<string>('');
  ...
  useEffect(() => {
    if (formData.tipoDeBusqueda) {
      setTipoDeBusqueda(formData.tipoDeBusqueda);
    }
  }, [formData]);
      ...
      <div className="fixed inset-0 flex justify-center items-center z-51 bg-black bg-opacity-50">
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            {/* Tipo de Busqueda */}
            <div className="mb-4">
              <label htmlFor="tipoDeBusqueda" className="block text-gray-700">
                Buscar por
              </label>

              <SimpleSelect
                options={tiposDeBusqueda}
                width={'100%'}
                value={formData.tipoDeBusqueda}
                onChange={(value) => {
                  setFormData((prev) => ({ ...prev, tipoDeBusqueda: value }));
                }}
              />

              {errors.tipoDeBusqueda && (
                <p className="text-red-500">{errors.tipoDeBusqueda}</p>
              )}
            </div>

            {/* Input dinámico basado en el tipo de búsqueda */}
            ...
            {tipoDeBusqueda === 'Numero de identificacion' && (
              <div className="mb-4">
                <label
                ...
                {errors.numeroIdentificacion && (
                  <p className="text-red-500">{errors.numeroIdentificacion}</p>
                )}
              </div>
            )}
...
export default BusquedaClienteModal;

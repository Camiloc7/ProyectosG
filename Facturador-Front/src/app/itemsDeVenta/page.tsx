...
const ItemsDeVenta: React.FC = () => {
  const { fetchListaDeItems, allItems, loading, deleteItem } =
    useItemsVentaStore();
  const { fetchCategoriasProductosVenta, categoriasVentas } =
    useDatosExtraStore();
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [listaDeItems, setListaDeItems] = useState<ItemsVentaFront[]>([]);
  ...
  const filteredItems = listaDeItems.filter((item) => {
          ...
          <div className="">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-4 justify-center sm:justify-start">
              <button
                ...
                onClick={() => showErrorToast('Boton aun en desarrollo.')}
              >
                CSV
              </button>
              <button
            ...
            {/* Tarjetas de items */}
            <div className="w-[100%] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 justify-items-center mt-10 ">

...
import { NotaCredito, NotaDebito } from '@/types/types';
...
const columnasIniciales = [
  { key: 'prefijoNotasDebito', label: 'PrefijoND', visible: true },
...
const ListaNotasDebito = () => {
  const { fetchNotasDeDebito, listaDeNotasDeDebito, loading } =
    useNotasDebitoStore();
  ...
  const filteredNotas = notas.filter((nota) => {
    ...
    // Filtro por búsqueda
    const matchesSearch =
      nota.prefijoNotaDebito.toLowerCase().includes(query) ||
  ...
  useEffect(() => {
    if (listaDeNotasDeDebito) {
      // console.log('Facturas antes de ordenar:', listaDeNotasDeDebito);

      const facturasOrdenadas = [...listaDeNotasDeDebito].sort((a, b) => {
    ...
    const sortedNotas = [...notas].sort((a, b) => {
      if (key === 'consecutivo' || key === 'prefijoNC' || key === 'valor') {
        return direction === 'asc'
          ? String(a[key as keyof NotaDebito]).localeCompare(
              String(b[key as keyof NotaDebito])
            )
          : String(b[key as keyof NotaDebito]).localeCompare(
              String(a[key as keyof NotaDebito])
            );
      }

      if (key === 'fecha') {
        const dateA = new Date(a[key as keyof NotaDebito] as string);
        const dateB = new Date(b[key as keyof NotaDebito] as string);
        return direction === 'asc'
  ...
  const handleCausativo = (id: string) => {
    ...
    const factura = listaDeNotasDeDebito.find((factura) => factura.id === id);
  ...
  const handleCopiar = (): void => {
    const datosCopiar =
      ...
      filteredNotas
        .map(
          (nota) =>
            `${nota.prefijoNotaDebito}   ${nota.consecutivo}  ${nota.fecha}  ${nota.valor}   ${nota.estado}`
        )
        .join('\n');
                        ...
                        .map((col) => (
                          <td key={col.key} className={estilosContenido}>
                            {col.key === 'factura' ? (
                              ...
                              nota[col.key as keyof NotaDebito]
                            )}
                          </td>
                        ))}
                    </tr>
              ...
              {notasActuales.map((nota) => (
                ...
                >
                  <p className="text-sm font-medium">
                    <strong>Prefijo:</strong> {nota.prefijoNotaDebito}
                  </p>
                  <p className="text-sm font-medium">
...
export default ListaNotasDebito;

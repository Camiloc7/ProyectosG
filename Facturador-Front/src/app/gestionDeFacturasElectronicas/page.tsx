...
import UpdateContractsWarning from '@/components/layout/UpdateContracts';

const yesOrNo: string[] = ['Si', 'No'];
const contratosFalsos: string[] = ['99999922', '8888333'];
...
type FormPrueba = {
  ...
  data_table: Array<{ descripcion: string; valor: number; cantidad: number }>;
  acta: string;
...
function parseListaItems(raw: string | undefined): Item[] {
  if (!raw) return [];

  try {
    const deserialized = unserialize(raw);

    // PHP devolvió un string con JSON (/s:.../), o un objeto
    if (typeof deserialized === 'string') {
      try {
        const arr = JSON.parse(deserialized);
        if (Array.isArray(arr)) {
          return arr.map((it: any) => normalizeItem(it));
        }
      } catch {
        console.warn('JSON.parse falló en LISTA_ITEMS:', deserialized);
        return [];
      }
    }

    if (typeof deserialized === 'object' && deserialized !== null) {
      return Object.values(deserialized).map((it: any) => normalizeItem(it));
    }

    return [];
  } catch (e) {
    console.error('Error al unserialize LISTA_ITEMS:', e);
    return [];
  }
}
...
function normalizeItem(raw: any): Item {
  return {
    ...
    valorUnitario: Number(raw.valor ?? raw.valorUnitario ?? 0),
    porcentajeIva: Number(raw.porcentajeIva ?? 0),
  };
}

export default function GestionDeFacturasElectronicas() {
  const {
    clientes,
    fetchClientes,
  ...
  const { sendFacturaData } = useFacturaPersistStore();
  // const showLoading = useFacturaPersistStore((s: any) => s.showLoading);
  // const setShowLoading = useFacturaPersistStore((s: any) => s.setShowLoading);
  const showLoading = useFacturaPersistStore((state) => state.showLoading);
  const startTime = useFacturaPersistStore((state) => state.startTime);
  const setShowLoading = useFacturaPersistStore(
    (state) => state.setShowLoading
  );
  ...
  //!DESCOMENTAR ESTO CUANDO SE VAYA A TRABAJAR EN EL MODAL DE REVIEW UX
  // useEffect(() => {
  ...
  //   if (currentCount >= 5) {
  //     localStorage.setItem('facturasGeneradas', (0).toString());
  //     setOpenReviewUX(true);
  //   }
  // }, []);
  ...
  useEffect(() => {
    if (facturaInfo) {
      const items = parseListaItems(facturaInfo.LISTA_ITEMS);
      setItems(items);

      setFormData((prev) => ({
        ...prev,
        contrato: facturaInfo.CONTRATO || '', //DEBO BUSCAR EL ID DEL CONTRATO Y PONERLO
        vencimiento: facturaInfo.VENCIMIENTO || '',
        tipoDeNegociacion: facturaInfo.TIPO_NEGOCIACION || '',
        medioDePago: facturaInfo.MEDIO_PAGO || '',
        tipoDeOperacion: facturaInfo.TIPO_OPERACION || '',
  ...
  }, [facturaInfo, clientes]);

  //CUANDO MODIFIQUEN EL CONTRATO DEBE PINTAR LOS DATOS QUE HAY EN DB
  useEffect(() => {
    const infoContrato = listaDeContratos.find(
      (e) => e.id === formData.contrato
    );

    if (infoContrato) {
      setFormData((prev) => ({
        ...prev,
        descripcionDelContrato: infoContrato.descripcion,
        // RETEFUENTE
        retefuente:
          Number(infoContrato.retefuente) !== 0 ? infoContrato.retefuente : '',
        retefuenteChecked: Number(infoContrato.retefuente) !== 0,
        // RETEICA
        reteica: Number(infoContrato.reteica) !== 0 ? infoContrato.reteica : '',
        reteicaChecked: Number(infoContrato.reteica) !== 0,
        // ADMINISTRACION
        porcentajeDeAdministracion:
          Number(infoContrato.administracion) !== 0
            ? infoContrato.administracion
            : '',
        adminChecked: Number(infoContrato.administracion) !== 0,
        // IMPREVISTOS
        imprevistos:
          Number(infoContrato.imprevistos) !== 0
            ? infoContrato.imprevistos
            : '',
        imprevistosChecked: Number(infoContrato.imprevistos) !== 0,
        // UTILIDAD
        porcentajeSobreUtilidad:
          Number(infoContrato.porcentajeUtilidad) !== 0
            ? infoContrato.porcentajeUtilidad
            : '',
        ivaChecked: Number(infoContrato.porcentajeUtilidad) !== 0,
        // Retegarantia
        retegarantia:
          Number(infoContrato.reteGarantia) !== 0
            ? infoContrato.reteGarantia
            : '',
        retegarantiaChecked: Number(infoContrato.reteGarantia) !== 0,
      }));
    }
  }, [formData.contrato]);
  ...
  const handleChange = (
    ...
    if (name === 'descripcion' && /[^a-zA-Z0-9\s.-]/.test(value)) {
      showErrorToast(
  ...
  const validateForm = (): boolean => {
    ...
    const isDescripcionValid =
      formData.descripcionDelContrato === '' // Solo valida si descripcionDelContrato está vacío
        ? isItemPresent || validateTextos(formData.descripcion) // Si no hay descripcionDelContrato, valida descripcion
  ...
  const handleSubmitButton = async (
    ...
    event.preventDefault();
    // console.log('oa');
    //!DECOMENTAR ESTO EN PRODUCCION
    if (validateForm()) {
      showErrorToast('Debe llenar los campos requeridos');
      return;
    }

    const formParaEnviar = prepareFormData();

    // const confirmado = window.confirm('Generar y enviar pdf?');
    ...
    if (confirmado) {
      setShowLoading(true);
      await sendFacturaData(formParaEnviar, () => {});
    }
  };
  ...
  const handleVistaPrevia = async (
    ...
    //!Esta es la logica para el modal review UX
    const currentCount = parseInt(
      localStorage.getItem('facturasGeneradas') || '0',
      10
    );
    ...
    localStorage.setItem('facturasGeneradas', newCount.toString());
  ...
  const handleAddItem = () => {
    ...
    const isDescripcionValid =
      validateTextos(formData.contrato) || validateTextos(formData.descripcion);
    ...
    if (hasErrors) {
      console.error('Errores detectados. No se agregara el item.', errors);
      return;
    }
    ...
    const newItem: Item = {
      descripcion: formData.descripcion
        ...
        : contracts.find((contract) => contract.id === formData.contrato)
            ?.nombre || '',
  ...
  const contrato = contracts.find(
    (contract) => contract.id === formData.contrato
  );
          ...
          <div className="w-full max-w-[1152px] h-auto rounded-[20px] flex justify-center px-4 sm:px-12 pb-14 mx-auto bg-white">
            ...
            <div className="w-full max-w-full md:max-w-[1061px] mt-8">
              ...
              <div className="h-auto">
                ...
                <h2 className="w-full min-[525px]:mt-[20px] min-[309px]:mt-[60px] min-[1px]:mt-[80px] mt-14 text-base font-montserrat font-normal text-[#6F6F6F] line-clamp-1">
              ...
              <div className="leading-[17.7px] font-montserrat font-normal text-[#6F6F6F] text-sm ">
                    ...
                    <div className="flex items-center w-full">
                      ...
                      <div className="w-[88%]">
                        {loadingFactura ? (
                          ...
                          <SimpleSelect
                            options={facturas.map((factura) => factura.nombre)}
                            width={'100%'}
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="w-full md:w-1/2 mt-4 ">
                    ...
                    <div className="flex">
                      ...
                      <div className="inline-flex items-center mr-3  mb-0 p-0 m-0 mt-4 ml-auto">
                        <button
                          className="w-10 ml-6 h-10 flex-shrink-0 flex items-center justify-center rounded-[20px] border border-[#00A7E1] hover:bg-blue-200 text-sm text-[#C3C3C3]"
                          onClick={() => setOpenBusquedaContrato(true)}
                        >
                          <IoIosSearch className="text-lg leading-none" />
                        </button>
                        <button
                          className="w-10 ml-3 h-10 flex-shrink-0 flex items-center justify-center rounded-[20px] border border-[#00A7E1] hover:bg-blue-200 text-2xl leading-none text-[#C3C3C3]"
                  ...
                  <div className="w-full md:w-1/2 mt-4 ">
                    ...
                    <div className="flex mt-4">
                      ...
                      />
                      <button
                        className="w-10 ml-6 h-10 flex-shrink-0 flex items-center justify-center rounded-[20px] border border-[#00A7E1] hover:bg-blue-200 text-sm leading-[2.5rem] text-[#C3C3C3]"
                        type="button"
                      ...
                      <button
                        className="w-10 ml-3 h-10 flex-shrink-0 flex items-center justify-center rounded-[20px] border border-[#00A7E1] hover:bg-blue-200 text-2xl leading-[2.5rem] text-[#C3C3C3]"
                        type="button"
                        onClick={() => setModalDeEnDesarrolloAbierto(true)}
                      >
                        +
                ...
                <div className="mt-4">
                  ...
                  <div className="bg-gray-200 rounded-[20px] p-4 mt-2 text-gray-700">
                    {formData.contrato ? (
                      <h1>
                        {contrato ? contrato.nombre : 'Contrato no encontrado'}
                      </h1>
                    ) : (
               ...
               md:top-0 top-[-4rem]" // Subir el botón en responsive
                  >
                    Agregar Item
                  </button>

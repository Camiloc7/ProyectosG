...
import YearsInput from '@/components/ui/TwoYearsInputModal';
import AYearInputModal from '@/features/compras/AYearInputModal';
import {
  showErrorToast,
  showTemporaryToast,
} from '@/components/feedback/toast';
import { EstadosCliente } from '../facturas/page';
...
const columnasIniciales = [
  ...
  { key: 'contrato', label: 'Código', visible: true },
...
const Compras = () => {
  const {
    ...
    importarFacturaCompra,
    checkEstadoFactura,
    aceptarRechazarFactura,
  } = useComprasStore();

  const { informeReteica, loading } = useComprasStore();
  const {
    anularFactura,
    sendFacturaMail,
    loading: loadingFactura,
  } = useFacturaStore();
  const { resumenDeCompras } = useInformesStore();
  ...
  const [reteicaOpen, setReteicaOpen] = useState(false);
  ...
  const [facturaImportada, setFacturaImportada] = useState<any>(null);
  ...
  const [datosDelEstadoFactura, setdatosDelEstadoFactura] = useState<
    EstadosCliente[] | null
  >(null);
  ...
  const verificarEstadoFactura = async () => {
    ...
    const datosDelEstadofactura = await checkEstadoFactura(cufes);
    setdatosDelEstadoFactura(datosDelEstadofactura);
  };
  ...
  const handleFileFacturasRecibido = (
    ...
    const file = event.target.files?.[0]; // Usa optional chaining para manejar posibles nulos.
    ...
    setFacturaImportada(file);
  };

  const handleContinuarImportar = () => {
    if (facturaImportada) {
      //console.log("factura ", facturaImportada);
      importarFacturaCompra(facturaImportada);
    }
    setImportarVisible(false); // Cierra el modal solo cuando se presiona "Continuar"
  };
  ...
  const estilosContenido =
    'px-2 py-2 text-[#6F6F6F] text-sm font-medium font-Inter h-[44px]';

  const estilosBoton =
    'h-5 w-16 text-xs font-medium font-inter text-[#00A7E1] border:none bg-[#E2F5FF] rounded-[16px] hover:bg-[#EDEFF3]';

  const estilosBotonAnular =
    'h-5 w-16 text-xs font-medium font-inter text-[#FFFFFF] bg-[#ffb2b2] rounded-[16px] hover:bg-[#FAD4D4]';
  ...
  const estilosBotonAceptar =
    'h-5 w-16 text-xs font-medium font-inter  text-green-700 border:none bg-green-100 rounded-[16px] hover:bg-green-200';
                        ...
                        .map((col) => (
                          <td key={col.key} className={estilosContenido}>
                            {col.key === 'factura' ? (
                              ...
                              (() => {
                                const estadoFactura =
                                  datosDelEstadoFactura?.[index];

                                if (estadoFactura?.ACEPTACION_EXPRESA === '1') {
                                  return (
                                  ...
                                  estadoFactura?.RECLAMO_FACTURA === '1'
                                ) {
                                  return (
                                  ...
                                  return (
                                    <div className=" flex gap-2 space-x-2 items-center justify-center">
                                      <button
                                        disabled={true}
                                        onClick={() => {
                                          return;
            ...
            <div className="block sm:hidden mt-8">
              {notasActuales.map((nota) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium">
                    <strong>Código:</strong> {nota.contrato}
                  </p>
                  <p className="text-sm font-medium">
                  ...
                  <form>
                    ...
                    <div className="flex justify-center gap-4 mt-4">
                      ...
                      <button
                        type="button"
                        className="bg-blueQ text-white h-11  hover:bg-[#008ec1] px-4 py-2 text-sm font-normal rounded-[25px]"
                        onClick={handleContinuarImportar}
                      >
                        Continuar
                      </button>
                    </div>
